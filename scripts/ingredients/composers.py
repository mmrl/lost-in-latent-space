...
class ActionAlignedMixer(nn.Module):
...
class Composer(nn.Module):
    def __init__(self, latent_size, n_actions, vae, mixing_layer):
        super().__init__()
        self.vae = vae
        self.projector = mixing_layer
    ...
    def forward(self, inputs):
        ...
        z_transf = self.projector(z, actions)
...
def init_composer(gm_type, n_actions, input_size, encoder_layers, latent_size,
                  mixing_layer='linear', decoder_layers=None):
    ...
    if mixing_layer == 'linear':
        mixing_layer = LinearMixer(n_actions, latent_size)
    elif mixing_layer == 'interp':
        mixing_layer = InterpolationMixer(n_actions, latent_size)
    elif mixing_layer == 'actalign':
        mixing_layer = ActionAlignedMixer(n_actions, latent_size)
    elif mixing_layer == 'mlp':
        mixing_layer = MLPMixer(n_actions, latent_size)

    return Composer(latent_size, n_actions, lgm, mixing_layer)
